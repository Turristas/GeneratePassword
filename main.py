...
import random
from env import TG_TOKEN#, TG_API_URL
...
def pass_is_lenght(message): #Какая длинна у пароля(-ей)?
    ...
    if is_lenght.isdigit() != True:
        ...
        bot.send_message(message.chat.id, text="Длинна у пароля(-ей) "+is_lenght+ ' символов. Договорились.'.format(message.from_user))
...
def pas_is_digits(message):#Включать-ли цифры
    ...
    if message.text == "Да" or message.text == "да":
        ...
        bot.send_message(message.chat.id, text='Включать ли прописные буквы ' + uppercase_letters + "? (да, нет)".format(message.from_user))
...
def pas_is_uppers(message):#Включать ли прописные буквы
    ...
    if message.text == "Да" or message.text == "да":
        chars.append(1)
        bot.send_message(message.chat.id, text="Прописные буквы " + uppercase_letters + ' будут в пароле.'.format(message.from_user))
        bot.send_message(message.chat.id, text='Включать ли строчные буквы ' + lowercase_letters + "? (да, нет)".format(message.from_user))
        bot.register_next_step_handler(message, pas_is_lowers)  # следующий шаг – функция pas_is_lowers
    elif message.text == "Нет" or message.text == "нет":
        chars.append(0)
        bot.send_message(message.chat.id, text="Прописных букв " + uppercase_letters + ' не будет в пароле.'.format(message.from_user))
        bot.send_message(message.chat.id,text='Включать ли строчные буквы ' + lowercase_letters + "? (да, нет)".format(message.from_user))
        bot.register_next_step_handler(message, pas_is_lowers)  # следующий шаг – функция pas_is_lowers
...
def pas_is_lowers(message):  # Включать ли строчные буквы
    ...
    if message.text == "Да" or message.text == "да":
        chars.append(1)
        bot.send_message(message.chat.id, text="Строчные буквы " + lowercase_letters + ' будут в пароле.'.format(message.from_user))
        bot.send_message(message.chat.id, text='Вкючать ли специальные символы ' + punctuation + "? (да, нет)".format(message.from_user))
        bot.register_next_step_handler(message, pas_is_punctuation)  # следующий шаг – функция pas_is_uppers
    elif message.text == "Нет" or message.text == "нет":
        chars.append(0)
        bot.send_message(message.chat.id, text="Строчных букв " + lowercase_letters + ' не будет в пароле.'.format(message.from_user))
        bot.send_message(message.chat.id,text='Вкючать ли специальные символы ' + punctuation + "? (да, нет)".format(message.from_user))
...
def pas_is_punctuation(message):#Вкючать ли специальные символы
    ...
    if message.text == "Да" or message.text == "да":
        chars.append(1)
        bot.send_message(message.chat.id, text="Специальные символы " + punctuation + ' будут в пароле.'.format(message.from_user))
        bot.send_message(message.chat.id, text='Вкючать ли однозначные символы ' + il1Lo0O + "? (да, нет)".format(message.from_user))
        bot.register_next_step_handler(message, pas_is_il1Lo0O)  # следующий шаг – функция pas_is_il1Lo0O
    elif message.text == "Нет" or message.text == "нет":
        chars.append(0)
        bot.send_message(message.chat.id, text="Специальных символов " + punctuation + ' не будет в пароле.'.format(message.from_user))
        bot.send_message(message.chat.id,text='Вкючать ли однозначные символы ' + il1Lo0O + "? (да, нет)".format(message.from_user))
        bot.register_next_step_handler(message, pas_is_il1Lo0O)  # следующий шаг – функция pas_is_il1Lo0O
...
def pas_is_il1Lo0O(message):
    ...
    if message.text == "Да" or message.text == "да":
        chars.append(1)
        bot.send_message(message.chat.id, text="Однозначные символы " + il1Lo0O + ' будут в пароле.'.format(message.from_user))
        ...
        chars.append(0)
        bot.send_message(message.chat.id, text="Однозначных символов " + il1Lo0O + ' не будет в пароле.'.format(message.from_user))
        bot.send_message(message.chat.id, text="Генерирую пароль...введите Ок".format(message.from_user))
        bot.register_next_step_handler(message, password)  # следующий шаг – функция password
...
def password(message):
    ...
    if message.text == "Ок":
        for i in range(int(is_quantity)):
            ...
            bot.send_message(message.chat.id, text=''.join(random.sample(pas_chars, int(is_lenght))).format(message.from_user))
